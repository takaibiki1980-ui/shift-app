...
### Step3: 夜勤アンカー配置最適化 **検証完了・不採用**
**目的**: 前半後半偏差の真の主因であるステップ1.5アンカー配置を改善する

**検証結果（100試行比較）**:

| 指標 | ベースライン | Step3改善版 | t値 | 判定 |
|---|---|---|---|---|
| 前半後半偏差（halfDev）| 0.854 | — | t=0.00 | ❌ 改善効果なし |
| 夜勤回数σ | 0.037 | — | — | — |
| 夜勤間隔σ | 1.216 | — | — | — |

**不採用理由**: 100試行比較で全指標 t=0.00。アンカー配置の前後半バランス調整は前半後半偏差に統計的有意な影響を与えなかった。

**Step3の結論**: 「アンカー配置の順序制御では前半後半偏差を改善できない」ことが実証された。halfDev の根本原因はアンカー順序ではなく別の要因にある。
