/**
 * Phase5 Step7-B' 本番autoGenerate Before 計測
 * shiftEngine.js を一時的に actualKyuko > targetKyuko (Before) の状態で実行
 *
 * 完全同一シード・条件で step7b_production.test.js (After) と比較可能。
 */
...
describe('Phase5 Step7-B Before: bestOfN 100試行×kaigo1/kaigo2 (shiftEngine.js > 条件)', () => {
  const beforeResults = { kaigo1:[], kaigo2:[] };
  ...
  test('100試行 実測', { timeout: 300000 }, () => {
    for (let sim=0; sim<SIMS; sim++) {
      for (const deptId of ['kaigo1','kaigo2']) {
        ...
        beforeResults[deptId].push({ kpi, score, elapsed });
      }
    }

    for (const deptId of ['kaigo1','kaigo2']) {
      const B = beforeResults[deptId];
      const fmt = (key, sub) => {
        const vals = B.map(r => sub ? r[key][sub] : r[key]);
        return { m:mean(vals), s:stddev(vals) };
      };
      console.log(`\n=== BEFORE: ${deptId} (n=${SIMS}) ===`);
    ...
    expect(beforeResults.kaigo1.length).toBe(SIMS);
    expect(beforeResults.kaigo2.length).toBe(SIMS);
  });
});
