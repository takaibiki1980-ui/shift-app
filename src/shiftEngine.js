  ...
  if (dept.shiftTypes.includes("夜勤")) {
    ...
    for (const s of sortedAnchorPool) {
      ...
      for (const D of kibodays) {
        ...
        res[s.id][nightDay] = "夜勤"; res[s.id][meakeDay] = "明け";
        lockedDays[s.id].add(nightDay); lockedDays[s.id].add(meakeDay);
      }
    }
  }
    ...
    for (let d = 1; d <= days; d++) {
      for (const [shiftKey, minCount] of Object.entries(dept.minStaff || {})) {
        ...
        for (const s of slideCands) {
          if (need <= 0) break;
          res[s.id][d] = shiftKey; need--; anyFixed = true; actual++;
        }
        if (need <= 0) continue;
...
export function scoreShifts(res, ds, dept, days, year, month, shiftTrend = {}) {
  ...
  for (const s of ds) {
    ...
    for (const t of workShiftTypes) {
      let sc = 0;
      for (let d = 1; d <= days; d++) {
        if (res[s.id]?.[d] === t) { sc++; if (sc === 4) score += 1500; else if (sc > 4) score += 6000; }
        else sc = 0;
      }
    }
  }
  ...
  if (ds.length > 1) {
    ...
    let varN = 0, varW = 0;
    for (const s of ds) { varN += (nc[s.id] - avgN) ** 2; varW += (wc[s.id] - avgW) ** 2; }
      ...
      for (const s of ds) {
        ...
        for (let d = 1; d <= days; d++) {
          ...
          if (LEARN_TYPES.has(shift)) {
            ...
            const predictedProb = dowRate ? (dowRate[shift] ?? 0) : (typeof trend[shift] === 'number' ? trend[shift] : 0);
