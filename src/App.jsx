...
//  勤務実績: 一覧・集計ビュー
// ─────────────────────────────────────────────
function JissekiView({ staffList, allJisseki, allShifts, dept, year, month, onCellClick, onCsvExport, onXlsExport, onBulkCopy, defaultTimes, onDefaultsChange }) {
  ...
  return (
    <div>
      ...
      <div style={{fontSize:11,color:"#5a9e9b",marginBottom:10,background:"#e0f4f2",borderRadius:7,padding:"6px 10px"}}>
        💡 スタッフ名横の <b>📋</b> でその人の予定を一括コピー。セルクリックで個別修正。<span style={{background:"#fff3b0",padding:"0 4px",borderRadius:3,marginLeft:4}}>黄色</span>＝デフォルト時刻から変更済み
      </div>
            ...
            {deptStaff.map((s,si)=>(
              <tr key={s.id} style={{background:si%2===0?"#fff":"#f7fffe"}}>
                ...
                {Array.from({length:days},(_,i)=>i+1).map(d=>{
                  ...
                  const mins=rec?calcWorkMinutes(rec.start,rec.end,rec.breakMin):0;
                  const hasRec=!!rec;
                  const modified=hasRec&&isModified(planned,rec);
                  return (
                    <td key={d} onClick={()=>onCellClick(s,d,planned)} style={{...TD({cursor:"pointer",background:modified?"#fff3b0":hasRec?"#d5f0ec":undefined,color:hasRec?"#0a4a47":"#b0c8c8"})}}>
