    ...
    <div style={{position:"fixed",inset:0,background:"#000000cc",zIndex:210,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"#FAFAFA",border:"1px solid #D4D4D8",borderRadius:14,padding:24,width:"100%",maxWidth:460,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 30px 80px #000"}}>
        ...
        {generationMode==='time'&&<div style={{background:"#f0f8ff",border:"1px solid #90c4e0",borderRadius:8,padding:"10px 12px",marginBottom:14}}>
          ...
          <div style={{marginTop:10,borderTop:"1px solid #cce5f5",paddingTop:8}}>
            <div style={{fontSize:11,color:"#3a6a87",fontWeight:700,marginBottom:4}}>最大配置人数の制約モード</div>
            <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>{[{val:true,label:"ソフト制約",desc:"充足不能時のみ超過を許容（栄養科など）"},{val:false,label:"ハード制約",desc:"絶対に超過しない（厳格運用向け）"}].map(({val,label,desc})=>(
              <label key={String(val)} style={{display:"flex",alignItems:"flex-start",gap:6,cursor:"pointer"}}>
                <input type="radio" checked={maxStaffRelaxable===val} onChange={()=>setMaxStaffRelaxable(val)} style={{marginTop:2,accentColor:"#6366F1"}}/>
                <div><div style={{fontSize:11,fontWeight:700,color:"#18181B"}}>{label}</div><div style={{fontSize:10,color:"#52525B"}}>{desc}</div></div>
              </label>
            ))}</div>
          </div>
        </div>}
