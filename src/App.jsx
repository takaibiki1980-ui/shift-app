...
function autoGenerate(staffList, dept, year, month, prevShifts, shiftTrend = {}) {
  ...
  if (dayTypes.length === 0) {
    ...
    { const _R=new Set(['休み','希望休','有休']); const _sh=ds.filter(s=>{const t=s.kyukoDaysByMonth?.[mk]??s.kyukoDays??8;return Object.values(res[s.id]).filter(v=>_R.has(v)).length<t;}); console.error(`[不足] PassA終了: ${_sh.length}名`); _sh.forEach(s=>{const t=s.kyukoDaysByMonth?.[mk]??s.kyukoDays??8;const days2=Object.entries(res[s.id]).filter(([,v])=>_R.has(v)).map(([d])=>+d).sort((a,b)=>a-b);const ake=Object.values(res[s.id]).filter(v=>v==='明け').length; const _lkRest=Object.values(res[s.id]).filter(v=>deptRest.has(v)&&v!=='明け').length; const _rtgt=Math.max(0,t-_lkRest); const _fxAke=Object.entries(res[s.id]).filter(([d,v])=>v==='休み'&&res[s.id][+d-1]==='明け').length; const _free=Array.from({length:days},(_,i)=>i+1).filter(d=>!res[s.id][d]); const _valid=_free.filter(d=>res[s.id][d-1]!=='明け'); console.error(`  ${s.name} target=${t} actual=${days2.length} diff=${days2.length-t} 明け=${ake} 休み日=[${days2.join(',')}]`); console.error(`  ↳[PassA診断] lockedRest=${_lkRest}(うち明け翌日休み=${_fxAke}) restTarget=${_rtgt} freeDays=${_free.length} validDays=${_valid.length}`); }); }
